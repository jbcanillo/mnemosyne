    ...
    try {
      if (newModelType === 'openrouter') {
        ...
        try {
          ...
          const exactMatch = checkResult.models.find(name => name.toLowerCase().includes(modelId.toLowerCase()));
  ...
  async function handleDeleteModel(modelId, type) {
    ...
    try {
      if (type === 'openrouter') {
        ...
        toast.error('Deleting Ollama models not supported via UI. Use: ollama rm ' + modelId);
   ...
   async function handleTestModel(modelId) {
    ...
    try {
      ...
      if (result.ok) {
        setTestResults(prev => ({ ...prev, [modelId]: { ok: true, reply: result.reply, tokens: result.tokens } }));
        toast.success(`Model "${modelId}" test successful! (${result.tokens} tokens)`);
      } else {
        setTestResults(prev => ({ ...prev, [modelId]: { ok: false, error: result.error } }));
        toast.error(`Test failed: ${result.error}`);
      }
    } catch (err) {
      setTestResults(prev => ({ ...prev, [modelId]: { ok: false, error: err.message } }));
      toast.error('Test failed: ' + err.message);
    } finally {
    ...
    <div className="settings-panel">
        ...
        <div className="sp-card-body">
          ...
          <div className="settings-form">
                 ...
                 <div className="form-group">
                   ...
                   <div className="key-input-wrap">
                     <input
                       ...
                       placeholder={settings?.openrouterApiKey ? 'Leave empty to keep current key' : 'sk-or-v1-…'}
                       autoComplete="off"
                       spellCheck={false}
                     />
                     ...
                     <button
                       ...
                       title={settings?.openrouterApiKey ? 'Test current saved API key' : 'No API key configured yet'}
                     >
                       {saving ? <span className="spinner-xs" /> : <Network size={14} />}
                       Test
                     </button>
                   </div>
                   <div className="form-hint">
                     ...
                     Changes to the key are saved with the main "Save Settings" button below.
                    </div>
                  </div>
            ...
            {showLocalSection && (
              <>
                <div className="form-group">
                  <label className="form-label">Active Local Ollama Model</label>
                  ...
                  {ollamaModelStatus === 'available' && (
                    <div className="form-hint" style={{ color: 'var(--color-success)' }}>✓ Model available</div>
                  )}

                  {ollamaModelStatus === 'not_found' && (
                    <div style={{ marginTop: '8px' }}>
                      <div className="form-hint" style={{ color: 'var(--color-danger)' }}>Model not found in Ollama</div>
                      <button
                        type="button"
                        className="btn btn-primary btn-sm"
            ...
            <div className="form-group">
              ...
              />
              <div className="form-hint">
                Defines the AI's behavior. Default enforces strict RAG (no hallucination).
              </div>
            </div>
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <div className="form-hint">Overlap between chunks. Default: 50</div>
              </div>
            </div>
      ...
      <div className="sp-card">
        ...
        <div className="sp-card-body">
          ...
          <div className="add-model-form" style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            <select
              className="sp-input"
              style={{ width: '150px' }}
              value={newModelType}
            ...
            <input
              ...
              placeholder={newModelType === 'openrouter' ? 'Model ID (e.g., openai/gpt-4o-mini:free)' : 'Ollama model (e.g., llama3.2)'}
              style={{ flex: 1 }}
            />
            <input
              className="sp-input"
            ...
            >
              {addingModel ? <span className="spinner-xs" /> : <Plus size={14} />} Add
            </button>
          </div>
          ...
          <div className="models-list">
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              {models.length === 0 ? (
                ...
                models.map(m => (
                  <div key={`or-${m.id}`} className="model-item" style={{ borderLeft: '3px solid var(--color-accent)' }}>
                     ...
                     <div className="model-actions">
                       <button
                         ...
                         title={m.active ? 'Cannot delete active model' : 'Delete'}
                       >
            ...
            <div>
              <div style={{ fontWeight: 'bold', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
               ...
               {ollamaModels.length === 0 ? (
                 <div className="models-empty">No Ollama models available. Pull models in the Ollama container.</div>
               ) : (
                  ollamaModels.map(m => (
                    <div key={`ollama-${m.name}`} className="model-item" style={{ borderLeft: '3px solid var(--color-success)' }}>
                      <div className="model-info">
                        <div className="model-name">
                          {m.name === localModel && <span className="model-active-badge">★ Active</span>}
                          {m.name}
                        </div>
                        <div className="model-id">Size: {(m.size / 1024 / 1024 / 1024).toFixed(2)} GB</div>
                      </div>
                      <div className="model-actions">
                        <button
                          className="btn btn-danger btn-sm"
                          onClick={() => handleDeleteModel(m.name, 'ollama')}
                          disabled={deletingModel === m.name || m.name === localModel}
                          title={m.name === localModel ? 'Cannot delete active model' : 'Delete'}
                        >
                          {deletingModel === m.name ? <span className="spinner-xs" /> : <Trash2 size={14} />}
                        </button>
                      </div>
                    </div>
                  ))
        ...
        <div className="sp-card-body">
          <div className="mgmt-grid">
            <div className="mgmt-card">
              ...
              <div className="mgmt-card-detail">Clears cached query results</div>
                  ...
                  {backups.map((b, i) => (
                    <div key={i} className="backup-item">
                      ...
                      <div className="backup-actions">
                        ...
                        <button className="btn btn-danger btn-xs" onClick={() => deleteBackup(b.filename)} title="Delete backup">
                          <Trash2 size={14} />
                        </button>
                       </div>
                     </div>
